...
class DatasetTrain(IterableDataset):
    def __init__(self, config, news_index, news_combined):
    ...
    def line_mapper(self, line):
        ...
        click_docs, log_mask = self.pad_to_fix_len(self.trans_to_nindex(click_docs), self.config['user_log_length'])
        user_feature = self.news_combined[click_docs]

        pos = self.trans_to_nindex(sess_pos)
        neg = self.trans_to_nindex(sess_neg)

        label = random.randint(0, self.config['npratio'])
    ...
    def __iter__(self):
        file_iter = open(self.config['processed_npbehavior_train_path'])
        return map(self.line_mapper, file_iter)


class DatasetTest(DatasetTrain):
    def __init__(self, config, news_index, news_scoring):
        super(DatasetTrain).__init__()
        self.config = config
        self.news_index = news_index
    ...
    def __iter__(self):
        file_iter = open(self.config['behavior_test_path'])
        return map(self.line_mapper, file_iter)
